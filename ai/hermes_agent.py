...
def run_hermes_agent(task: str, model: str = "nous-hermes2") -> dict:
    ...
    if target_ip is None:
        ...
        if cve_search_term:
            print(f"Step 7: Looking up CVEs for signature '{cve_search_term}'...")
            cve_result = lookup_cve(cve_search_term)
            steps.append({"step": 7, "tool": "lookup_cve", "input": cve_search_term, "result": cve_result, "status": "done"})
        else:
        ...
        print(f"Step 8: Correlating Zeek data for {target_ip}...")
        zeek_result = correlate_zeek(target_ip)
        steps.append({"step": 8, "tool": "correlate_zeek", "input": target_ip, "result": zeek_result, "status": "done"})
